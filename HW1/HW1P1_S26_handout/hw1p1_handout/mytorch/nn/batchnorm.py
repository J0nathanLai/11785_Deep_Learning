...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.M = np.mean(self.Z, axis=0, keepdims=True)  # TODO: Calculate mini-batch mean
        self.V = np.var(self.Z, axis=0, keepdims=True)  # TODO: Calculate mini-batch variance
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -1/2 * np.sum(dLdNZ * (self.Z - self.M) * (self.V + self.eps)**(-3/2), axis=0)  # TODO: Compute gradient of loss backprop through variance calculation.
        dNZdM = -(self.V + self.eps)**(-1/2) - 1/2 * (self.Z - self.M) * (self.V + self.eps)**(-3/2) * (-2/self.N * (np.sum(self.Z - self.M, axis=0)))  # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM, axis=0)  # TODO: Compute gradient of loss with respect to mean.

        dLdZ = dLdNZ * ((self.V + self.eps)**(-1/2)) + dLdV * (2/self.N * (self.Z - self.M)) + (1/self.N)*dLdM  # TODO: Compute gradient of loss with respect to the input.
